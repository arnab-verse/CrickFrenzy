...
git clone https://github.com/<your-username>/crick-frenzy.git
cd crick-frenzy
...
This compiles all React and TypeScript code into a single, optimized **`dist/`** directory containing static HTML, CSS, and JavaScript.
...
2. In the [Cloudflare Dashboard](https://dash.cloudflare.com/):
   ...
   - Select your `crick-frenzy` repository.
...
### 3. Deploying to GitHub Pages / Netlify / Vercel

- **GitHub Pages**: You can use GitHub Actions to deploy the `dist/` directory directly to `gh-pages`.
- **Netlify**: Connect repository, set build command to `npm run build`, and publish directory to `dist`.
- **Vercel**: Framework preset `Vite`, root directory `./`, output directory `dist`.
